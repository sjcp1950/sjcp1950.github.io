...
//section
	...
	$(".box").each(function(box_i, box) {
		var box_attr = $(box).children().attr("box");
		if ( box_attr ) {
			box_attr = box_attr.split(",");
			for (var i=0; i<box_attr.length; i++) {
				var attr_name = $.trim( box_attr[i].split("=")[0] );
				var attr_content = $.trim( box_attr[i].split("=")[1] );
				$(box).attr(attr_name, attr_content);
			}
		}
		$(box).children().removeAttr("box");
	});
	$("body").children().wrapAll("<div class='main grid'></div>");
//	$("body").children().wrapAll("<div class='main'><div class='grid'></div></div>");
...
// navigator
	$(".main").before(
		"<nav>"+
			...
			"</ul>"+
		"</nav>");
	$("nav").find("li").children("a[href='" + pathname + "']").attr("select", "");
	$("nav").find("a[select]").parent("li").parent("ul").prev("a").attr("select", "");
